...
IMPLEMENT_SHADER_TYPE(, FCubismMaskVS, TEXT("/Plugin/UnLive2DAsset/Private/CubismMask.usf"), TEXT("MainVS"), SF_Vertex)
IMPLEMENT_SHADER_TYPE(, FCubismMaskPS, TEXT("/Plugin/UnLive2DAsset/Private/CubismMask.usf"), TEXT("MainPS"), SF_Pixel)
    ...
    RHICmdList.BeginRenderPass(RPInfo, TEXT("DrawMask01"));
    {
        ...
        for (csmUint32 clipIndex = 0; clipIndex < _clippingManager->_clippingContextListForMask.GetSize(); clipIndex++)
        {
            ...
            for (csmInt32 i = 0; i < clipDrawCount; i++)
            {
                ...
                {
                    UE_LOG(LogUnLive2D, Warning, TEXT("DrawSeparateToRenderTarget_RenderThread:[Mask] Texture %d invalid"), td_TextrueNo);
                    continue;
                }
