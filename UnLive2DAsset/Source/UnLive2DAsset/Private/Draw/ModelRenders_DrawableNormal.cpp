...
IMPLEMENT_SHADER_TYPE(, FCubismSepVS, TEXT("/Plugin/UnLive2DAsset/Private/CubismSep.usf"), TEXT("MainVS"), SF_Vertex)
IMPLEMENT_SHADER_TYPE(, FCubismSepPS, TEXT("/Plugin/UnLive2DAsset/Private/CubismSep.usf"), TEXT("MainPS"), SF_Pixel)
...
)
{
    ...
    if (!tp_Texture || !IsValid(tp_Texture))
    {
        return;
    }
    ...
    RHICmdList.BeginRenderPass(RPInfo, TEXT("DrawSeparate"));
    {
        ...
        //////////////////////////////////////////////////////////////////////////
        /** Drawable draw */
        FIndexBufferRHIRef* IndexBufferRHI = tp_States->IndexBuffers.Find(drawableIndex);
        FVertexBufferRHIRef* ScratchVertexBufferRHI = tp_States->VertexBuffers.Find(drawableIndex);

        if (ScratchVertexBufferRHI == nullptr || IndexBufferRHI == nullptr)
		{
            return;
		}

        if (!tp_Texture->Resource)
        {
            return;
        }
